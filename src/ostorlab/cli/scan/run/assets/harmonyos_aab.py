...
def harmonyos_aab(
    ...
    """Run scan for HarmonyOS .AAB package file."""
    ...
    if file != ():
        for f in file:
            assets.append(
                harmonyos_aab_asset.HarmonyOSAab(content=f.read(), path=str(f.name))
            )
    if url != ():
        for u in url:
            assets.append(harmonyos_aab_asset.HarmonyOSAab(content_url=u))
