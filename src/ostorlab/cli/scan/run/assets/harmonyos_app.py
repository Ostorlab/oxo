"""Asset of type .APP package file.
This module takes care of preparing a file of type .APP before injecting it to the runtime instance.
"""
...
def harmonyos_app(
    ...
    if url != () and file != ():
        console.error("Command accepts either path or source url of the app file.")
        raise click.exceptions.Exit(2)
    if url == () and file == ():
        console.error("Command missing either file path or source url of the app file.")
        raise click.exceptions.Exit(2)

    if file != ():
        for f in file:
            assets.append(
                harmonyos_app_asset.HarmonyOSApp(content=f.read(), path=str(f.name))
            )
    if url != ():
        for u in url:
            assets.append(harmonyos_app_asset.HarmonyOSApp(content_url=u))
