...
def harmonyos_apk(
    ...
    """Run scan for HarmonyOS .APK package file."""
    ...
    if file != ():
        for f in file:
            assets.append(
                harmonyos_apk_asset.HarmonyOSApk(content=f.read(), path=str(f.name))
            )
    if url != ():
        for u in url:
            assets.append(harmonyos_apk_asset.HarmonyOSApk(content_url=u))
