"""Asset of type .HAP package file.
This module takes care of preparing a file of type .HAP before injecting it to the runtime instance.
"""
...
def harmonyos_hap(
    ...
    """Run scan for HarmonyOS .HAP package file."""
    runtime = ctx.obj["runtime"]
    assets = []

    if url != () and file != ():
        console.error("Command accepts either path or source url of the hap file.")
        raise click.exceptions.Exit(2)
    if url == () and file == ():
        console.error("Command missing either file path or source url of the hap file.")
        raise click.exceptions.Exit(2)

    if file != ():
        for f in file:
            assets.append(
                harmonyos_hap_asset.HarmonyOSHap(content=f.read(), path=str(f.name))
            )
    if url != ():
        for u in url:
            assets.append(harmonyos_hap_asset.HarmonyOSHap(content_url=u))
