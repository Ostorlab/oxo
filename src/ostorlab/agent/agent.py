...
def _is_transient_network_error(exception: BaseException) -> bool:
    """Return True if the exception, or any exception in its cause/context chain, is a transient network error.

    LLM SDKs (litellm, openai, pydantic_ai) and HTTP libraries wrap low-level connection failures in their own
    exception types while chaining the original httpx/socket error via ``__cause__``. Walking that chain lets the
    framework detect transient connection errors without depending on every wrapping library.
    """
    seen: set[int] = set()
