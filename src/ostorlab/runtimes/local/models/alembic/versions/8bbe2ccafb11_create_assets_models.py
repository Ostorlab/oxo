...
def upgrade() -> None:
    ...
    op.create_table('android_file',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('package_name', sa.String(length=255), nullable=True),
    sa.Column('path', sa.String(length=1024), nullable=True),
    sa.ForeignKeyConstraint(['id'], ['asset.id'], name=op.f('fk_android_file_id_asset')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_android_file'))
    )
    op.create_table('android_store',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('package_name', sa.String(length=255), nullable=True),
    sa.Column('application_name', sa.String(length=255), nullable=True),
    sa.ForeignKeyConstraint(['id'], ['asset.id'], name=op.f('fk_android_store_id_asset')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_android_store'))
    )
    op.create_table('ios_file',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('bundle_id', sa.String(length=255), nullable=True),
    sa.Column('path', sa.String(length=1024), nullable=True),
    sa.ForeignKeyConstraint(['id'], ['asset.id'], name=op.f('fk_ios_file_id_asset')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_ios_file'))
    )
    op.create_table('ios_store',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('bundle_id', sa.String(length=255), nullable=True),
    sa.Column('application_name', sa.String(length=255), nullable=True),
    sa.ForeignKeyConstraint(['id'], ['asset.id'], name=op.f('fk_ios_store_id_asset')),
    ...
    with op.batch_alter_table('scan', schema=None) as batch_op:
        ...
        batch_op.create_foreign_key(batch_op.f('fk_scan_asset_id_asset'), 'asset', ['asset_id'], ['id'])
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('scan', schema=None) as batch_op:
        batch_op.drop_constraint(batch_op.f('fk_scan_asset_id_asset'), type_='foreignkey')
