...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['network_asset_id'], ['network.id'], name=op.f('fk_ip_network_asset_id_network')),
    ...
    sa.ForeignKeyConstraint(['urls_asset_id'], ['urls.id'], name=op.f('fk_link_urls_asset_id_urls')),
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('network', schema=None) as batch_op:
        batch_op.add_column(sa.Column('networks', sa.VARCHAR(length=1024), nullable=True))
