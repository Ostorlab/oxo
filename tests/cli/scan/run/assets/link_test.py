...
def testScanRunLink_whenNoOptionsProvided_showsAvailableOptionsAndCommands(mocker):
    ...
    result = runner.invoke(rootcli.rootcli, ['scan', 'run', '--agent=agent1 --agent=agent2', 'link'])

    assert 'Usage:' in result.output
    assert result.exit_code == 2


def testRunScanLink__whenValidAgentsAreProvidedWithNoAsset_ShowSpecifySubCommandError(mocker):
    ...
    result = runner.invoke(rootcli.rootcli,
                           ['scan', '--runtime=local', 'run', '--agent=agent1 --agent=agent2', 'link', '--url',
                            'https://ostorlab.co', '--method', 'GET'])
