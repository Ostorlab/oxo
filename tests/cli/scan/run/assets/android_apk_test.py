...
def testScanRunAndroidApk_whenNoOptionsProvided_shouldExitAndShowError(mocker):
    ...
    result = runner.invoke(rootcli.rootcli, ['scan', 'run', '--agent=agent1 --agent=agent2', 'android-apk'])

    assert 'Command missing either file path or source url of the apk file.' in result.output
    assert result.exit_code == 2


def testScanRunAndroidApk_whenBothFileAndUrlOptionsAreProvided_shouldExitAndShowError(mocker):
    ...
    mocker.patch('ostorlab.runtimes.local.LocalRuntime.__init__', return_value=None)
    mocker.patch('ostorlab.runtimes.local.LocalRuntime.can_run', return_value=True)
    command=['scan', 'run', '--agent=agent1', 'android-apk', '--file', 'android_apk_test.py', '--url', 'url1']
